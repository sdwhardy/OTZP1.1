...
#################### functions ###################
function form_EllipseConstraint(Elips)
    toPlot=processDual(value(Elips))
    dia_Major,XY0,XY1=get_MajorAxis(toPlot)
    r_x,r_y,cntrXY,xyMinor,xyMajor,offset=get_ElipseData(toPlot,XY0,XY1,dia_Major)
    return r_x,r_y,cntrXY,offset
end

function findLargestEllipse(poly_g)
    _solver = with_optimizer(Mosek.Optimizer, QUIET=false)
...
function get_ElipseData(tp,maXY0,maXY1,maDia)
    ...
    if (!((rad <= maDia/2+0.001) && (rad >= maDia/2-0.001)))
        ...
        if verln==true
            ...
            offset=atan((mn_xy.y-centre_xy.y)/(mn_xy.x-centre_xy.x))
            r_x=deepcopy(rad)
            r_y=deepcopy(maDia/2)
        else
            #maDia/2=x rad=y
            ...
            offset=atan((maXY1.y-centre_xy.y)/(maXY1.x-centre_xy.x))
            r_y=deepcopy(rad)
            r_x=deepcopy(maDia/2)
        end
    #is a circle
    else
    ...
    return r_x,r_y,centre_xy,mn_xy,maXY1,offset
end
